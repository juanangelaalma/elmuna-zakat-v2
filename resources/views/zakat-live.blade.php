...
<head>
    ...
    <style>
        ...
        .text-fluid-xs {
            font-size: clamp(.625rem, .65vw, 1rem);
        }
        ...
        .text-fluid-base {
            font-size: clamp(.875rem, .95vw, 1.35rem);
        }
        ...
        .text-fluid-xl {
            font-size: clamp(1.125rem, 1.3vw, 1.9rem);
        }
        ...
        .text-fluid-4xl {
            font-size: clamp(1.875rem, 2.8vw, 4.2rem);
        }

        .text-fluid-5xl {
            font-size: clamp(2.25rem, 3.5vw, 5.5rem);
        }
        ...
        /* ── Icon scale ── */
        .icon-fluid {
            font-size: clamp(1.25rem, 1.8vw, 2.8rem) !important;
        }
        ...
        /* ── Dot / ring scale ── */
        .dot-fluid {
            width: clamp(.5rem, .7vw, 1rem);
            height: clamp(.5rem, .7vw, 1rem);
        }
        ...
        .gap-fluid {
            gap: clamp(.75rem, 1.2vw, 2rem);
        }

        /* ── Rounded scale ── */
        .rounded-fluid {
            border-radius: clamp(.75rem, 1.2vw, 1.75rem);
        }
    </style>
</head>
...
<body class="bg-bg-light text-text-dark font-display min-h-screen lg:h-screen lg:max-h-screen flex flex-col overflow-y-auto lg:overflow-hidden selection:bg-primary/20 selection:text-primary">
                ...
                <div class="flex-1 overflow-y-auto min-h-0">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody id="breakdown-table" class="divide-y divide-slate-100">
                            <tr>
                                <td colspan="3" class="p-4 text-center text-text-muted text-fluid-sm">Memuat data…</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    <footer class="bg-white border-t border-slate-200 flex items-center overflow-hidden z-30 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.04)] shrink-0"
        ...
        <div class="shrink-0 hidden sm:flex items-center bg-slate-50 h-full border-l border-slate-200 text-slate-400 text-fluid-xs font-medium italic"
            ...
            Built with <span class="text-red-400 not-italic">❤</span> by&nbsp;<span class="text-primary font-semibold not-italic">Remas El Muna</span>
        </div>
    </footer>
    ...
    <script>
        ...
        function updateClock() {
            ...
            // Hijriah (Intl API)
            try {
                const hijriah = now.toLocaleDateString('id-ID-u-ca-islamic-umalqura', {
                    day: 'numeric',
                    month: 'long',
                    year: 'numeric'
                });
        ...
        function renderToday(today) {
            ...
            setText('today-rice', today.total_rice_kg.toLocaleString('id-ID', {
        ...
        function renderStock(stock) {
            ...
            setText('stock-total', total.toLocaleString('id-ID', {
                minimumFractionDigits: 1
            }));
            setText('stock-pct-label', pct + '% Tersedia');
            setText('stock-bar-label', avail.toLocaleString('id-ID', {
                minimumFractionDigits: 1
            }) + ' kg');
        ...
        function renderOverall(overall) {
            ...
            setText('overall-rice', overall.total_rice_kg.toLocaleString('id-ID', {
                minimumFractionDigits: 1
            }) + ' kg');
            setText('overall-rice-small', overall.total_rice_kg.toLocaleString('id-ID', {
                minimumFractionDigits: 1
            }) + ' kg');
