...
export default function RiceSales() {
    const { data, totalAmount, totalQuantity, numberOfRecords } = usePage().props as any;

    const handleExport = () => {
        window.location.href = '/transactions/details/rice-sales/export';
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Penjualan Beras" />
                        ...
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                                    Daftar Penjualan Beras
                                </h2>
                                <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                                    Daftar semua penjualan beras
                                </p>
                            </div>
